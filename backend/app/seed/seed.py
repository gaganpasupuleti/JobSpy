...
def seed_database(db: Session) -> dict:
    ...
    profiles_created = _seed_search_profiles(db)
...
def _seed_search_profiles(db: Session) -> int:
    created = 0
    roles = db.query(RoleCategory).all()
    locations = db.query(Location).filter(Location.is_active.is_(True)).all()
    roles_data = json.loads((SEED_DIR / "roles_keywords.json").read_text(encoding="utf-8"))
    search_terms = {r["slug"]: r["search_term"] for r in roles_data["role_categories"]}
        ...
        for location in locations:
            ...
            db.add(
                SearchProfile(
                    ...
                    experience_band_id=None,
                    search_term=search_terms.get(role.slug, role.name),
